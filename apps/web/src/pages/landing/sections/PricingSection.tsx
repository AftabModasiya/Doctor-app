...
import { FaCheck, FaTimes, FaArrowRight, FaStar } from 'react-icons/fa';
...
const plans = [
  {
    ...
    badge: '',
    ctaClass: 'border border-primary-600 text-primary-600 hover:bg-primary-50',
    features: [
  ...
  },
  {
    ...
    color: 'border-primary-500 shadow-primary-100',
    badge: 'Most Popular',
    highlight: true,
    ctaClass: 'bg-primary-600 hover:bg-primary-700 text-white shadow-lg shadow-primary-200',
    features: [
  ...
  },
  {
    ...
    color: 'border-gray-200',
    badge: '',
    ctaClass: 'border border-gray-800 text-gray-800 hover:bg-gray-50',
    features: [
    ...
    <section id="pricing" className="py-24 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-16">
          <span className="inline-block text-xs font-bold text-amber-600 bg-amber-50 px-4 py-1.5 rounded-full uppercase tracking-widest mb-4">
            Pricing
          </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
          {plans.map(({ name, price, period, description, color, badge, highlight, ctaClass, features }) => (
            <div
              key={name}
              className={clsx(
                'relative bg-white rounded-3xl border-2 p-8 transition-all duration-300',
                color,
                highlight ? 'shadow-xl shadow-primary-100' : 'shadow-sm hover:shadow-md hover:-translate-y-0.5'
              )}
            >
              {/* Popular Badge */}
              {badge && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                  <span className="inline-flex items-center gap-1.5 bg-primary-600 text-white text-xs font-bold px-4 py-1.5 rounded-full shadow-sm">
                    <FaStar className="h-3 w-3" /> {badge}
                  </span>
                </div>
              )}
                ...
                {features.map(({ text, included }) => (
                  <li key={text} className="flex items-center gap-3">
                    {included ? (
                      ...
                      <FaTimes className="h-3.5 w-3.5 text-gray-300 flex-shrink-0" />
                    )}
                    <span className={clsx('text-sm', included ? 'text-gray-700' : 'text-gray-400')}>
                      {text}
                    </span>
                  </li>
                ))}
